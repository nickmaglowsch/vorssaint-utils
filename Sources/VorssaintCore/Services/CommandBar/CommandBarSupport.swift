...
enum CommandBarSearch {
    ...
    static func pinyinKeywords(_ title: String) -> String {
        // Was CFStringTransform(_:_:kCFStringTransformMandarinLatin,_:) inline;
        // WP-12 put it behind `Transliterator` so this file could move into the
        // platform-free core. The guard is the same one: no transform, or a
        // romanization equal to the title, means no keywords.
        guard let romanized = Transliteration.current.mandarinLatin(title),
              romanized != title else { return "" }
        let syllables = Transliteration.current.strippingDiacritics(romanized)
