...
| Every rule reaches the same decisions as the macOS Support type it is a port of | `tests/test_rules.c`: 147 assertions carried over from `Tests/MetricsTests.swift`, 136 of them with the Swift's own message text so the two suites can be diffed. See `docs/linux-port/RELAY_RULES.md` for the per-rule counts and for every vector that was *not* ported, with the reason |
