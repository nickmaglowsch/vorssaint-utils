...
| 1 Shared core | Package split, file moves, Platform protocols, Combine shim, settings store, catalog flags, `swift test` on both platforms, Linux CI leg | **Passed 2026-09-12.** WP-10 to WP-18 all merged. `VorssaintCore` is 130 files / 50 136 lines building on Swift 6.1.3 Linux with 138 tests green; the macOS gate reports `TESTS OK (31565 checks)`, the WP-10 baseline unchanged, on every push. A third CI job compiles a C client against the bridge header alone and runs it against a real static library. |
