...
| WP-14/WP-15 | review | plan branch | `SettingsStore` with `UserDefaultsSettingsStore` (macOS, a forwarder) and `JSONSettingsStore` (`$XDG_CONFIG_HOME/vorssaint/settings.json`, atomic write + fsync + rename, debounced flush, a damaged file moved aside and reported); all 644 keys round-trip on both legs and a backup crosses platforms both ways. `FeatureSupportCatalog`: 57 features, 48 on Linux, the nine Drop rows hidden, per-feature `PlatformCapability` requirements from `FEATURE_TRIAGE.md`; `isSupportedOnThisPlatform` replaces `isHardwareSupported`; three Linux presets; eight hub strings in thirteen languages. macOS stays at exactly 31565 checks. Two new CI drift guards (`defaults-inventory.py --verify`, `check-feature-catalog.py`). Docs: `SETTINGS.md`, the WP-15 section of `FEATURE_TRIAGE.md`, `BRIDGE.md` § 6 |
...
| WP-A1..A4 | merged (backend) | plan branch | /proc, /sys, hwmon, UPower over sd-bus with a sysfs fallback, three GPU vendors, three fixture trees; five green matrix legs |
