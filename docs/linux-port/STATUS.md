...
| WP-C1 | review | worktree | five backends (X11 and sway measured; Hyprland, KWin, GNOME against fakes); KWin scripts cannot own a bus name, so commands load as generated scripts and answer to a sink we own; QA running | C window backends: X11 EWMH, wlr foreign-toplevel + Sway IPC, Hyprland IPC, KWin script bridge; `vorssaint_platform.h` contract |
