...
**Result.** `Sources/VorssaintCore` holds **97 files / 44 389 lines** of real
Vorssaint code and `swift build --target VorssaintCore` is green on Linux
Swift 6.1.3. `Sources/Vorssaint` keeps 350 files / 148 424 lines, of which
103 138 lines are outside `UI/`. The macOS product is the same set of sources
compiled into the same single module: `build.sh` globs
`Sources/Vorssaint`, `Sources/VorssaintCore` and `Sources/VorssaintMac`
together, so a file crossing that boundary changes nothing for the Mac build.
...
```
$ python3 /tmp/wp11/census_cmp.py          # script inlined in the report
...
| The census counts imports, not APIs. `PointerTapRunLoop.swift` imports only Foundation and uses `CFRunLoopGetCurrent`, `CFMachPortInvalidate` and `NSMachPort`. | `/tmp/wp11/hazard.py` output, § 4 below |
...
### `Core/Defaults.swift` → split, the key table moved (`5b70939`-line, commit `ecd5546`'s parent)
...
```
$ python3 /tmp/wp11/gsproof.py            # script inlined in the report
```

30 non-UI files reference it. **26 need the Carbon/CGEvent half** (a `kVK_`
default, `carbonKeyCode`/`carbonModifiers`, `cgFlags`,
`syntheticEventFlags`, `matches(event:)`, `displayString`, or
`GlobalShortcut(storageValue:)` which routes through `keyLabel`). The four
that use only the value type — `App/AppDelegate.swift`,
`Services/CommandBar/CommandBarCatalog.swift`,
`Services/CommandBar/CommandBarRowShortcuts.swift`,
...
|---|---|---:|---|
| `…DefaultsKey split` | `Core/DefaultsKeys.swift` | 1 | the split itself; `build.sh --test` path |
...
Only **three** source edits exist in the whole work package, and all three are
import lines:
...
`Localization.swift` (`AppLanguage`, `L10n`, `Strings`), the twelve
