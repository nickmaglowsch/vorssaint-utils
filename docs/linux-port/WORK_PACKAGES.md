...
| WP-12 | Platform protocol layer | L | WP-11 | Core porter | merged (both gates green on 5cdbfbd, run 34693623124; `TESTS OK (31565 checks)` unchanged; 14 protocols + fakes + macOS adapters; 6 seams; core 130 files / 50 136 lines; VorssaintCombine deleted; PLATFORM.md) |
...
| WP-14 | Settings store abstraction (UserDefaults ↔ JSON/GSettings) | M | WP-12 | Core porter | merged (644 keys round-tripped both stores; backup crosses platforms on both legs) |
| WP-15 | Feature catalog: platform support flags and Linux presets | S | WP-12 | Core porter | merged (57 features, 48 on Linux, 32 capability requirements, 3 Linux presets) |
...
| ID | Title | Size | after | Owner role | Status |
|---|---|---|---|---|---|
| WP-20 | Linux executable skeleton (`linux/shell` Qt Quick app + `VorssaintLinux` core), `CoreModel` QML binding, main loop, single-instance, CLI flags (`--selftest`) | M | WP-18, WP-01 | Shell squad | partial: the WP-01 app is promoted to `linux/shell` (executable, `CoreModel`, three screens, app id/icon/`.desktop`), links the real bridge by default, and has `--version`/`--selftest` (bridge, sampler, snapshot round trip, all three screens' QML). **Remaining:** single-instance guard, D-Bus activation, settings persistence, `Strings` plumbing in all thirteen languages |
| WP-21 | Tray: StatusNotifierItem host with per-readout items, GNOME AppIndicator detection | M | WP-20 | Shell squad | partial: one `QSystemTrayIcon` item with the app icon and a three-entry menu, proven to register over D-Bus against the WP-01 fake watcher in CI (`linux/shell/tests/smoke-tray.sh`). **Remaining:** per-readout items, live icon/label updates, GNOME AppIndicator detection and the honest message when no watcher exists |
...
| WP-P1 | Linux build script `build-linux.sh` + AppImage recipe: linuxdeploy + plugin-qt on the oldest supported glibc with a pinned Qt, the hand-built AppDir from `spikes/wp04-packaging` kept as the audit implementation and diffed in CI, excludelist including libpipewire/libwayland-client/libglvnd, `platformthemes` bundled, launcher names the libglvnd package when `libEGL.so.1` is missing | M | WP-04, WP-20 | Packaging/CI | partial: `linux/packaging/build-appimage.sh` (linuxdeploy + plugin-qt, `platformthemes` and the Wayland sub-plugins bundled) runs on ubuntu-22.04/glibc 2.35 in the `shell-appimage` job and uploads the AppImage as a workflow artifact; one container leg (`shell-smoke-noble`) downloads it and runs it on ubuntu:24.04. **Remaining:** `build-linux.sh`, a pinned Qt instead of the distro's, the hand-built AppDir diffed against the linuxdeploy one in CI, the libglvnd launcher message, and the rest of WP-P3's distro matrix |
...
| WP-D6 | quitWindowProtection (Ctrl+Q / Ctrl+W hold, double press, modifier) — the helper-side rule landed with WP-D2, gated by `SetContext`; what remains is the app half: settings, the HUD, and pushing the focused `app_id` from WP-C1. **Owns the `SetContext` Swift side**: WP-12's `InputInterceptor` mirrors the helper's D-Bus API but has no counterpart for `SetContext(focused_app_id)` (WP-12 review, defect 3), because nothing could push a focused app id until `WindowSystem` had a Linux backend. Add the method to `InputInterceptor` here and wire it from the window backend's focus events | S | WP-D1, WP-C1 | todo |
