...
| WP-14 | Settings store abstraction (UserDefaults ↔ JSON/GSettings) | M | WP-12 | Core porter | review (`SettingsStore` + `UserDefaultsSettingsStore` + `JSONSettingsStore`; 644 keys round-trip on both legs; cross-platform backup test; `SETTINGS.md` with the migration list) |
| WP-15 | Feature catalog: platform support flags and Linux presets | S | WP-12 | Core porter | review (`FeatureSupportCatalog`: 57 features, 48 on Linux, capabilities per feature; `isSupportedOnThisPlatform`; 3 Linux presets; 8 strings × 13 languages; WP-15 section of `FEATURE_TRIAGE.md`) |
