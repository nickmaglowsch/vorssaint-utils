...
LOGS="$STATE/logs"
CFG="$STATE/config"
OUTPUT_NAME=${WP02_OUTPUT:-HEADLESS-1}
OUTPUT_MODE=${WP02_MODE:-1280x720@60Hz}
SINK_NAME=${WP02_SINK:-wp02-null-sink}
HERE=$(cd "$(dirname "$0")" && pwd)

# Two things this container forces on us, both documented in 02-capture.md:
#
# WP02_XDPW: which xdg-desktop-portal-wlr binary to run. Stock 0.7.1 refuses
...
#   fixed build; set WP02_XDPW=/usr/libexec/xdg-desktop-portal-wlr to see the
#   stock failure instead.
#
# WP02_ACCESS_SHIM: xdg-desktop-portal will not export the Screenshot portal
...
PATCHED_XDPW=${WP02_XDPW_PREFIX:-/tmp/wp02/xdpw-patched}/libexec/xdg-desktop-portal-wlr
XDPW_BIN=${WP02_XDPW:-$([ -x "$PATCHED_XDPW" ] && echo "$PATCHED_XDPW" || echo /usr/libexec/xdg-desktop-portal-wlr)}
ACCESS_SHIM=${WP02_ACCESS_SHIM:-1}
...
write_configs() {
  ...
  cat > "$CFG/pipewire/pipewire.conf.d/10-wp02-null-sink.conf" <<EOF
context.objects = [
  { factory = adapter
    args = {
...
EOF
    ...
    wait_for 100 dbus-send --session --dest=org.freedesktop.DBus --print-reply \
        /org/freedesktop/DBus org.freedesktop.DBus.GetNameOwner \
        string:org.freedesktop.impl.portal.desktop.wp02mock \
...
export WP02_OUTPUT=$OUTPUT_NAME
export WP02_SINK=$SINK_NAME
EOF
...
status() {
  ...
  pw-cli ls Node 2>/dev/null | grep -A1 -i "$WP02_SINK" | head -20
}
