...
(cd minimal && swift test)                  # URLCleaning + CommandBarMath, a package of its own
```
...
- `files-minimal.txt` — `URLCleaning.swift` and `CommandBarMath.swift`: two files that declare everything they use. They live in `minimal/`, a package of their own, because `swift test` builds every target in a package and the parent deliberately contains targets that do not compile.
